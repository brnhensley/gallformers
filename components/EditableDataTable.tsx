...
type EditableCellProps<T extends WithID> = {
    ...
    columnKey: keyof T;
    validate?: (row: T, field: keyof T, value: string) => Promise<ValidationResult>;
};
 ...
 */
const EditableCell = <T extends WithID>({ row, rowIndex, col, onChange, columnKey, validate }: EditableCellProps<T>) => {
    const originalValue = getDataFromSelector(row, rowIndex, col);
    const [value, setValue] = useState<string>(originalValue);
    const [editing, setEditing] = useState(false);

    const onBlur = async () => {
        if (validate) {
            const result = await validate(row, columnKey, value);
            if (!result.valid) {
