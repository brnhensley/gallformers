...
## HTMX Development (Server-Rendered HTML)

V2 includes HTMX + templ infrastructure for server-rendered pages. This coexists with the Svelte SPA.
...
# Run with templ hot reload for HTMX development
make dev-htmx
```
...
api/
├── cmd/server/
│   └── assets/           # Embedded static assets (HTMX, Alpine, branding)
├── internal/
│   ├── handlers/
│   │   └── htmx_example.go  # Reference HTMX handler implementation
│   └── views/
...
Handlers should detect HTMX requests and return appropriate responses:
...
func (h *Handler) Page(w http.ResponseWriter, r *http.Request) {
    ...
    if r.Header.Get("HX-Request") == "true" {
        ...
        pages.PageContent(data).Render(r.Context(), w)
    } else {
        // Return full page with layout
        pages.FullPage(data).Render(r.Context(), w)
    }
}
```
...
Each page should have two templates:

```go
// Full page with layout (for initial load)
templ FullPage(data Data) {
    @layouts.Base(data.Title) {
        @PageContent(data)
    }
}

// Just content (for HTMX partial updates)
templ PageContent(data Data) {
    <div>...</div>
}
```
...
Common HTMX response headers are available in `handlers/htmx_example.go`:

- `isHTMXRequest(r)` - Check if request is from HTMX
- `isHTMXBoosted(r)` - Check if request is from hx-boost
- `setHTMXPushURL(w, url)` - Update browser URL
- `setHTMXRedirect(w, url)` - Client-side redirect
- `setHTMXRetarget(w, target)` - Change swap target

### Tailwind with HTMX Templates

The base template uses Tailwind CDN with custom config for brand colors. In production, we'll compile Tailwind CSS with proper purging.

### Testing HTMX Routes

Visit `http://localhost:8080/htmx/example` to see the example page. Click "Refresh Items" to test HTMX partial updates.
